$(() => {
  ...
  const addListingByCategory = function(categoryArray) {

    for (let obj of categoryArray) {
  ...
  const category = location.href.split('/')[4];
  console.log(category);
  $.ajax(`/api/categories/${category}`, { method: 'GET' })
    .then(res => {
      console.log(res);
      addListingByCategory(res.result);
    })
    .catch(err => {
      console.log(err);
    })
